  ...
  var p = new Promise(function (resolve, reject) {
    ...
    if (nowData.expire < now + 10 * 1000) {
      policy()
        .then(data => {
          nowData = data
          const formData = genFormData(file, path)
          axios
            .post(nowData.host, formData)
            .then(response => {
...
function genKey (file, path) {
  ...
  if (file.name === undefined) {
    extensionName = '.' + file.type.substr(file.type.indexOf('/') + 1) // 文件扩展名
  } else {
    extensionName = file.name.substr(file.name.indexOf('.')) // 文件扩展名
  }
  const fileName = path + '/' + dateTime + '_' + randomStr + extensionName // 文件名字（相对于根目录的路径 + 文件名）
  return fileName
}
